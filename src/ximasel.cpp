...
bool CxImage::SelectionAddRect(RECT r, uint8_t level)
{
	...
	if (info.rSelectionBox.top <= r2.top) info.rSelectionBox.top = std::max(0,(int32_t)std::min(head.biHeight,r2.top+1));
	if (info.rSelectionBox.left > r2.left) info.rSelectionBox.left = std::max(0,(int32_t)std::min(head.biWidth,r2.left));
	if (info.rSelectionBox.right <= r2.right) info.rSelectionBox.right = std::max(0,(int32_t)std::min(head.biWidth,r2.right+1));
	if (info.rSelectionBox.bottom > r2.bottom) info.rSelectionBox.bottom = std::max(0,(int32_t)std::min(head.biHeight,r2.bottom));

	int32_t ymin = std::max(0,(int32_t)std::min(head.biHeight,r2.bottom));
	int32_t ymax = std::max(0,(int32_t)std::min(head.biHeight,r2.top+1));
	int32_t xmin = std::max(0,(int32_t)std::min(head.biWidth,r2.left));
	int32_t xmax = std::max(0,(int32_t)std::min(head.biWidth,r2.right+1));
...
bool CxImage::SelectionAddEllipse(RECT r, uint8_t level)
{
	...
	if (info.rSelectionBox.left > (xcenter - xradius)) info.rSelectionBox.left = std::max(0,std::min((int32_t)head.biWidth,(xcenter - xradius)));
	if (info.rSelectionBox.right <= (xcenter + xradius)) info.rSelectionBox.right = std::max(0,std::min((int32_t)head.biWidth,(xcenter + xradius + 1)));
	if (info.rSelectionBox.bottom > (ycenter - yradius)) info.rSelectionBox.bottom = std::max(0,std::min((int32_t)head.biHeight,(ycenter - yradius)));
	if (info.rSelectionBox.top <= (ycenter + yradius)) info.rSelectionBox.top = std::max(0,std::min((int32_t)head.biHeight,(ycenter + yradius + 1)));

	int32_t xmin = std::max(0,std::min((int32_t)head.biWidth,(xcenter - xradius)));
	int32_t xmax = std::max(0,std::min((int32_t)head.biWidth,(xcenter + xradius + 1)));
	int32_t ymin = std::max(0,std::min((int32_t)head.biHeight,(ycenter - yradius)));
	int32_t ymax = std::max(0,std::min((int32_t)head.biHeight,(ycenter + yradius + 1)));
...
bool CxImage::SelectionAddPolygon(POINT *points, int32_t npoints, uint8_t level)
{
	...
	while (i < npoints){
		...
		if (localbox.top < r2.top) localbox.top = std::max(0,(int32_t)std::min(head.biHeight-1,r2.top+1));
		if (localbox.left > r2.left) localbox.left = std::max(0,(int32_t)std::min(head.biWidth-1,r2.left-1));
		if (localbox.right < r2.right) localbox.right = std::max(0,(int32_t)std::min(head.biWidth-1,r2.right+1));
		if (localbox.bottom > r2.bottom) localbox.bottom = std::max(0,(int32_t)std::min(head.biHeight-1,r2.bottom-1));
