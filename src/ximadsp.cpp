...
bool CxImage::Light(int32_t brightness, int32_t contrast)
{
	...
	for (int32_t i=0;i<256;i++)	{
		cTable[i] = (uint8_t)std::max(0,std::min(255,(int32_t)((i-128)*c + brightness + 0.5f)));
	}
			...
			for(int32_t x=xmin; x<xmax; x++, iY1++){
...
#endif //CXIMAGE_SUPPORT_SELECTION
				{
					...
					if (Kfactor==0 || ksumcur==0){
						cPtr2[iY1] = (uint8_t)std::min(255, std::max(0,(int32_t)(b + Koffset)));
					} else if (ksumtot == ksumcur) {
						cPtr2[iY1] = (uint8_t)std::min(255, std::max(0,(int32_t)(b/Kfactor + Koffset)));
					} else {
						cPtr2[iY1] = (uint8_t)std::min(255, std::max(0,(int32_t)((b*ksumtot)/(ksumcur*Kfactor) + Koffset)));
					}
				}
			}
		}
		...
		for(int32_t y=ymin; y<ymax; y++){
			...
			for(int32_t x=xmin; x<xmax; x++){
				...
				if (BlindSelectionIsInside(x,y))
					...
					if (Kfactor==0 || ksumcur==0){
						c.rgbRed   = (uint8_t)std::min(255, std::max(0,(int32_t)(r + Koffset)));
						c.rgbGreen = (uint8_t)std::min(255, std::max(0,(int32_t)(g + Koffset)));
						c.rgbBlue  = (uint8_t)std::min(255, std::max(0,(int32_t)(b + Koffset)));
					} else if (ksumtot == ksumcur) {
						c.rgbRed   = (uint8_t)std::min(255, std::max(0,(int32_t)(r/Kfactor + Koffset)));
						c.rgbGreen = (uint8_t)std::min(255, std::max(0,(int32_t)(g/Kfactor + Koffset)));
						c.rgbBlue  = (uint8_t)std::min(255, std::max(0,(int32_t)(b/Kfactor + Koffset)));
					} else {
						c.rgbRed   = (uint8_t)std::min(255, std::max(0,(int32_t)((r*ksumtot)/(ksumcur*Kfactor) + Koffset)));
						c.rgbGreen = (uint8_t)std::min(255, std::max(0,(int32_t)((g*ksumtot)/(ksumcur*Kfactor) + Koffset)));
						c.rgbBlue  = (uint8_t)std::min(255, std::max(0,(int32_t)((b*ksumtot)/(ksumcur*Kfactor) + Koffset)));
					}
...
{
    int32_t lWide = std::min(GetWidth(),imgsrc2.GetWidth()-lXOffset);
    int32_t lHeight = std::min(GetHeight(),imgsrc2.GetHeight()-lYOffset);
        ...
        for(int32_t lX=0;lX<lWide;lX++)
        {
...
#endif //CXIMAGE_SUPPORT_SELECTION
			{
				...
				switch(op)
				{
					...
					break;
					case OpAdd:
						rgbDest.rgbBlue = (uint8_t)std::max(0,std::min(255,rgb1.rgbBlue+rgb2.rgbBlue));
						rgbDest.rgbGreen = (uint8_t)std::max(0,std::min(255,rgb1.rgbGreen+rgb2.rgbGreen));
						rgbDest.rgbRed = (uint8_t)std::max(0,std::min(255,rgb1.rgbRed+rgb2.rgbRed));
						if (bEditAlpha) rgbDest.rgbReserved = (uint8_t)std::max(0,std::min(255,rgb1.rgbReserved+rgb2.rgbReserved));
					break;
					case OpSub:
						rgbDest.rgbBlue = (uint8_t)std::max(0,std::min(255,rgb1.rgbBlue-rgb2.rgbBlue));
						rgbDest.rgbGreen = (uint8_t)std::max(0,std::min(255,rgb1.rgbGreen-rgb2.rgbGreen));
					...
					case OpSrcBlend:
						...
						else
						{
							...
							if( lAverage < lThresh+1){
								rgbDest.rgbBlue = (uint8_t)std::max(0,std::min(255,(int32_t)(dLarge*((double)rgb1.rgbBlue) +
												dSmallAmt)));
								rgbDest.rgbGreen = (uint8_t)std::max(0,std::min(255,(int32_t)(dLarge*((double)rgb1.rgbGreen) +
												dSmallAmt)));
								rgbDest.rgbRed = (uint8_t)std::max(0,std::min(255,(int32_t)(dLarge*((double)rgb1.rgbRed) +
												dSmallAmt)));
							}
							else
								rgbDest = rgb1;
					...
					case OpBlendAlpha:	//[brunom]
						if(rgb2.rgbReserved != 0)
						{
							...
							if( (rgb1.rgbReserved < 5) || (rgb2.rgbReserved > 250) ){
								...
								rgbDest.rgbBlue		= (uint8_t)((rgb2.rgbBlue	* a2	+ a1 * rgb1.rgbBlue	)/a0);
								rgbDest.rgbGreen	= (uint8_t)((rgb2.rgbGreen * a2	+ a1 * rgb1.rgbGreen)/a0);
								rgbDest.rgbRed		= (uint8_t)((rgb2.rgbRed	* a2	+ a1 * rgb1.rgbRed	)/a0);
							}
						} else {
							rgbDest = rgb1;
	...
	if (head.biClrUsed==0){
		...
		for(int32_t y=ymin; y<ymax; y++){
			for(int32_t x=xmin; x<xmax; x++){
				...
				{
					color = BlindGetPixelColor(x,y);
					color.rgbRed = (uint8_t)std::max(0,std::min(255,(int32_t)(color.rgbRed + r)));
					color.rgbGreen = (uint8_t)std::max(0,std::min(255,(int32_t)(color.rgbGreen + g)));
					color.rgbBlue = (uint8_t)std::max(0,std::min(255,(int32_t)(color.rgbBlue + b)));
					BlindSetPixelColor(x,y,color);
				}
			}
		}
...
bool CxImage::Gamma(float gamma)
{
	...
	uint8_t cTable[256]; //<nipper>
	for (int32_t i=0;i<256;i++)	{
		cTable[i] = (uint8_t)std::max(0,std::min(255,(int32_t)( pow((double)i, dinvgamma) / dMax)));
	}
...
bool CxImage::GammaRGB(float gammaR, float gammaG, float gammaB)
{
	...
	uint8_t cTableR[256];
	for (i=0;i<256;i++)	{
		cTableR[i] = (uint8_t)std::max(0,std::min(255,(int32_t)( pow((double)i, dinvgamma) / dMax)));
	}
	...
	uint8_t cTableG[256];
	for (i=0;i<256;i++)	{
		cTableG[i] = (uint8_t)std::max(0,std::min(255,(int32_t)( pow((double)i, dinvgamma) / dMax)));
	}
	...
	uint8_t cTableB[256];
	for (i=0;i<256;i++)	{
		cTableB[i] = (uint8_t)std::max(0,std::min(255,(int32_t)( pow((double)i, dinvgamma) / dMax)));
	}
	...
	for(int32_t y=ymin; y<ymax; y++){
		...
		for(int32_t x=xmin; x<xmax; x++){
...
#endif //CXIMAGE_SUPPORT_SELECTION
			{
				...
				color.rgbRed = (uint8_t)std::max(0,std::min(255,(int32_t)(color.rgbRed + n)));
				n=(int32_t)((rand()/(float)RAND_MAX - 0.5)*level);
				color.rgbGreen = (uint8_t)std::max(0,std::min(255,(int32_t)(color.rgbGreen + n)));
				n=(int32_t)((rand()/(float)RAND_MAX - 0.5)*level);
				color.rgbBlue = (uint8_t)std::max(0,std::min(255,(int32_t)(color.rgbBlue + n)));
				BlindSetPixelColor(x,y,color);
			}
		}
	}
				   ...
				   int32_t direction, bool bForceFFT, bool bMagnitude)
{
	...
	//DFT buffers
	double *real2,*imag2;
	real2 = (double*)malloc(std::max(w,h) * sizeof(double));
	imag2 = (double*)malloc(std::max(w,h) * sizeof(double));
	...
	double nn=pow((double)2,(double)log((double)std::max(w,h))/(double)log((double)2)-4);
	...
	for (j=0;j<h;j++) {
		for (k=0;k<w;k++) {
			if (bMagnitude){
				tmpReal->SetPixelIndex(k,j,(uint8_t)std::max(0,std::min(255,(int32_t)(nn*(3+log(_cabs(grid[k][j])))))));
				if (grid[k][j].x==0){
					tmpImag->SetPixelIndex(k,j,(uint8_t)std::max(0,std::min(255,(int32_t)(128+(atan(grid[k][j].y/0.0000000001)*nn)))));
				} else {
					tmpImag->SetPixelIndex(k,j,(uint8_t)std::max(0,std::min(255,(int32_t)(128+(atan(grid[k][j].y/grid[k][j].x)*nn)))));
				}
			} else {
				tmpReal->SetPixelIndex(k,j,(uint8_t)std::max(0,std::min(255,(int32_t)(128 + grid[k][j].x*nn))));
				tmpImag->SetPixelIndex(k,j,(uint8_t)std::max(0,std::min(255,(int32_t)(128 + grid[k][j].y*nn))));
			}
		}
	}
...
bool CxImage::RepairChannel(CxImage *ch, float radius)
{
	...
	for(x=1; x<w; x++){
		for(y=1; y<h; y++){
			...
			tmp.BlindSetPixelIndex(x,y,(uint8_t)std::min(255,(int32_t)std::max(0.,(xy0 + radius * correction + 0.5))));
		}
	}
...
#endif //CXIMAGE_SUPPORT_SELECTION
			{
				for (int32_t b=0, z=x*bypp; b<bypp; b++, z++){
					...
					if (abs(diff) < threshold){
						...
						dest_row[z] = (uint8_t)std::min(255, std::max(0,(int32_t)(cur_row[z] + amount * diff)));
					}
				}
			}
		}
	...
	for(int32_t y=ymin; y<ymax; y++){
		...
		for(int32_t x=xmin; x<xmax; x++){
...
#endif //CXIMAGE_SUPPORT_SELECTION
			{
				...
				if (a<0) a=0;
				color = BlindGetPixelColor(x,y);
				color.rgbRed = (uint8_t)(a*std::min(color.rgbGreen,color.rgbBlue)+(1.0f-a)*color.rgbRed);
				BlindSetPixelColor(x,y,color);
			}
		}
	}
...
bool CxImage::Saturate(const int32_t saturation, const int32_t colorspace)
{
	...
	case 1:
		{
			for (int32_t i=0;i<256;i++)	{
				cTable[i] = (uint8_t)std::max(0,std::min(255,(int32_t)(i + saturation)));
			}
			for(int32_t y=ymin; y<ymax; y++){
				info.nProgress = (int32_t)(100*(y-ymin)/(ymax-ymin));
				if (info.nEscape) break;
	...
	case 2:
		{
			for (int32_t i=0;i<256;i++)	{
				cTable[i] = (uint8_t)std::max(0,std::min(255,(int32_t)((i-128)*(100 + saturation)/100.0f + 128.5f)));
			}
			for(int32_t y=ymin; y<ymax; y++){
				info.nProgress = (int32_t)(100*(y-ymin)/(ymax-ymin));
				if (info.nEscape) break;
...
int32_t  CxImage::OptimalThreshold(int32_t method, RECT * pBox, CxImage* pContrastMask)
{
	...
	int32_t xmin,xmax,ymin,ymax;
	if (pBox){
		xmin = std::max((long)pBox->left,0L);
		xmax = std::min(pBox->right,head.biWidth);
		ymin = std::max((long)pBox->bottom,0L);
		ymax = std::min(pBox->top,head.biHeight);
	} else {
		xmin = ymin = 0;
		xmax = head.biWidth; ymax=head.biHeight;
	}
...
bool CxImage::AdaptiveThreshold(int32_t method, int32_t nBoxSize, CxImage* pContrastMask, int32_t nBias, float fGlobalLocalBalance)
{
	...
	for (int32_t y=0; y<mh; y++){
		for (int32_t x=0; x<mw; x++){
			...
			mask.SetPixelIndex(x,y,(uint8_t)std::max(0.,
			        std::min(255.,nBias+((1.0-fGlobalLocalBalance)*threshold + fGlobalLocalBalance*globalthreshold))));
		}
	}
...
//------------------------------------- Begin of Flood Fill
	...
	if (IsIndexed()){ //--- Generic indexed image, no tolerance OR Grayscale image with tolerance
		...
		uint8_t idxMin = (uint8_t)std::min(255, std::max(0,(int32_t)(idxRef - nTolerance)));
		uint8_t idxMax = (uint8_t)std::min(255, std::max(0,(int32_t)(idxRef + nTolerance)));
		...
		cRefMin.rgbRed   = (uint8_t)std::min(255, std::max(0,(int32_t)(cRef.rgbRed   - nTolerance)));
		cRefMin.rgbGreen = (uint8_t)std::min(255, std::max(0,(int32_t)(cRef.rgbGreen - nTolerance)));
		cRefMin.rgbBlue  = (uint8_t)std::min(255, std::max(0,(int32_t)(cRef.rgbBlue  - nTolerance)));
		cRefMax.rgbRed   = (uint8_t)std::min(255, std::max(0,(int32_t)(cRef.rgbRed   + nTolerance)));
		cRefMax.rgbGreen = (uint8_t)std::min(255, std::max(0,(int32_t)(cRef.rgbGreen + nTolerance)));
		cRefMax.rgbBlue  = (uint8_t)std::min(255, std::max(0,(int32_t)(cRef.rgbBlue  + nTolerance)));
