...
bool CxImage::HistogramStretch(int32_t method, double threshold)
{
  ...
  if ((head.biBitCount==8) && IsGrayScale()){
	...
	uint8_t lut[256];
	for (x = 0; x <256; x++){
		lut[x] = (uint8_t)std::max(0,std::min(255,(255 * (x - minc) / (maxc - minc))));
	}
	...
	case 2:
	  { // <nipper>
		...
		if (range != 0)	{
			for (x = 0; x <256; x++){
				lutR[x] = (uint8_t)std::max(0,std::min(255,(255 * (x - minR) / range)));
			}
		} else lutR[minR] = minR;
		...
		if (range != 0)	{
			for (x = 0; x <256; x++){
				lutG[x] = (uint8_t)std::max(0,std::min(255,(255 * (x - minG) / range)));
			}
		} else lutG[minG] = minG;
		...
		if (range != 0)	{
			for (x = 0; x <256; x++){
				lutB[x] = (uint8_t)std::max(0,std::min(255,(255 * (x - minB) / range)));
			}
		} else lutB[minB] = minB;
