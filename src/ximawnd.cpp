...
int32_t CxImage::Draw(HDC hdc, int32_t x, int32_t y, int32_t cx, int32_t cy, RECT* pClipRect, bool bSmooth, bool bFlipY)
{
	...
	paintbox.top = std::min(clipbox.bottom,std::max(clipbox.top,(long)y));
	paintbox.left = std::min(clipbox.right,std::max(clipbox.left,(long)x));
	paintbox.right = std::max(clipbox.left,std::min(clipbox.right,(long)(x+cx)));
	paintbox.bottom = std::max(clipbox.top,std::min(clipbox.bottom,(long)(y+cy)));
