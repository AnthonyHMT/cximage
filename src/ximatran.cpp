	...
	for (int32_t y=0;y<head.biHeight;y++){
		...
		for (int32_t x=0;x<head.biWidth;x++){
			if (!errordiffusion){
				...
				c.rgbRed = (uint8_t)std::min(255,std::max(0,(int32_t)c.rgbRed + ((er*7)/16)));
				c.rgbGreen = (uint8_t)std::min(255,std::max(0,(int32_t)c.rgbGreen + ((eg*7)/16)));
				c.rgbBlue = (uint8_t)std::min(255,std::max(0,(int32_t)c.rgbBlue + ((eb*7)/16)));
				...
				for(int32_t i=-1; i<2; i++){
					...
					c.rgbRed = (uint8_t)std::min(255,std::max(0,(int32_t)c.rgbRed + ((er * coeff)/16)));
					c.rgbGreen = (uint8_t)std::min(255,std::max(0,(int32_t)c.rgbGreen + ((eg * coeff)/16)));
					c.rgbBlue = (uint8_t)std::min(255,std::max(0,(int32_t)c.rgbBlue + ((eb * coeff)/16)));
